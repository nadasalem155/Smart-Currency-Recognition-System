...
@st.cache_resource
def load_models():
    # Load the SVM classifier, the standard scaler, and the PCA transformer
...
def auto_crop_currency(img_bgr):
    ...
    if contours:
        # Filter the largest area which is likely the banknote
        largest_contour = max(contours, key=cv2.contourArea)
        x, y, w, h = cv2.boundingRect(largest_contour)
        
        # Minimum size validation to avoid cropping small noise
        if w > 50 and h > 50:
            return img_bgr[y:y+h, x:x+w]
            
    return img_bgr # Return original if no clear note is detected

def extract_features(img_bgr):
    """
    Extracts HOG (Texture) and HSV Histograms (Color) features.
    """
    # 1. Image Resizing for consistency
    ...
    # 3. HSV Color Feature Extraction
...
def speak(text):
    ...
    # Encode audio to base64 to embed in HTML for autoplay
...
# User selection for input source
...
# ======================
# If an image exists, process and predict immediately without a button
if raw_img is not None:
    ...
    with st.spinner("Analyzing..."):
        ...
        # 3. Optional: Expand to see the cropped result
